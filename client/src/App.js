...
function App() {
  ...
  const [userLocation, setUserLocation] = useState(() => {
    const savedLocation = JSON.parse(localStorage.getItem("My_Personal_Address"));
    return savedLocation || initialUserAddress;
  });

  const [userEducation, setUserEducation] = useState(() => user_education);
  ...
  const [workExperience, setWorkExperience] = useState(() => user_work_experience);
